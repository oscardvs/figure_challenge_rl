...
A submission for an RL browser-agent challenge. The target is a 30-step sequential web navigation gauntlet: each step shows a puzzle (scroll, click, hover, drag, shadow DOM, canvas, audio, websockets and so on) that reveals a 6-character code, and the agent has to find the code, enter it and submit to reach the next step.

The repository contains a deterministic Playwright solver, an LLM agent that works from BrowserGym observations, and a training pipeline (SFT, DPO, M-GRPO) that fine-tunes Qwen3-4B on trajectories recorded from the solver.
...
```bash
# API mode: an LLM agent solves challenges via API calls
...
The deterministic solver acts as a teacher. It runs Playwright to solve challenges while a `RecordingPage` proxy intercepts every call and translates it to BrowserGym action format. This produces labeled trajectories with no API cost.
...
Uses an LLM agent with Monte Carlo Tree Search to explore challenges. More expensive, but it covers challenge types the deterministic solver can't handle.
...
LoRA rank (`r`) controls the number of trainable parameters. Higher rank means more capacity but more overfitting risk, especially with small datasets.

| Rank | Alpha | Trainable params | % of 4B | Notes |
|------|-------|-----------------|---------|-------|
| r=16 | 32 | 33M | 1.3% | Default; suits fewer than 500 trajectories |
...
**Guidelines** (from the [Unsloth LoRA Hyperparameters Guide](https://unsloth.ai/docs/get-started/fine-tuning-llms-guide/lora-hyperparameters-guide)):
- Keep `alpha = 2 * rank` or `alpha = rank`
- Training loss below 0.2 signals overfitting; reduce epochs or increase dropout
- 1-3 epochs recommended; more than 3 epochs gives diminishing returns with small data
- `lora_dropout=0.05` helps regularize with fewer than 500 examples
...
Online RL with live browser rollouts. The model generates multiple trajectories per challenge, receives rewards based on progress, and updates via a clipped importance-weighted loss (no value network needed).
...
M-GRPO uses `GauntletEnv` with randomized challenges and curriculum learning: it starts with early steps and expands as the model improves. To run the trained model, see [Local Mode](#local-mode---mode-local) below.
...
figure_challenge_rl/
├── config/
│   ├── challenge_config.yaml        # Challenge URL, step count, MCTS params, challenge types
│   └── model_config.yaml            # Base model, QLoRA, training hyperparams
├── src/
│   ├── agent/
│   │   ├── policy.py                # LLM wrappers (Anthropic, OpenAI, Gemini) + LocalPolicy
...
│   │   └── vision_fallback.py       # Gemini screenshot fallback when heuristics fail
│   ├── runner/
│   │   ├── solve_all.py             # Main entry point: solve the full gauntlet
│   │   ├── metrics.py               # Per-step and aggregate metrics
│   │   └── parallel_executor.py     # Parallel challenge execution with vLLM batching
│   └── training/
...
├── data/                            # Gitignored except challenge_taxonomy.json
...
├── models/                          # Trained model checkpoints (sft/, dpo/, grpo/); gitignored
├── results/                         # Run metrics and logs; gitignored
├── scripts/
│   ├── debug_run.py                 # Smoke test: 15 actions on step 1
│   ├── explore_challenges.py        # Manual challenge exploration
│   └── test_transition.py           # Solve step 1, check the step 2 observation
├── .env                             # API keys (gitignored)
├── run.sh                           # One-command entry point
└── requirements.txt
```

The `*_to_port.py` and `browser_for_porting.py` files at the repository root are unchanged copies of the earlier `figure_challenge` agent (`agent_solver.py`, `agent_vision.py`, `dom_parser.py`, `browser.py`), kept as reference while porting its handlers.

## How It Works

### The Gauntlet

The target is a React SPA with 30 sequential puzzle steps. Each step randomly assigns one of about 21 challenge types:
...
Codes are 6-character alphanumeric strings (e.g., `K7SMGA`), generated fresh on each run. Direct URL navigation only works for step 1; all other steps require sequential traversal from the start.

### Architecture

**Deterministic Solver** (`src/solver/`): Pattern-matching approach. A `ChallengeDetector` collects DOM signals via a single `page.evaluate()` call and classifies the challenge type. Then a type-specific handler from `ChallengeHandlers` executes the solve strategy using raw Playwright. Covers 21 challenge types.
...
All parameters are in `config/model_config.yaml`. The tables below reflect the values currently in that file.
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| `max_seq_length` | 4096 | Compact observations: AXTree of about 1200 tokens plus 2000 characters of HTML |
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| `beta` | 0.05 | KL penalty weight, kept low for RL agents |
| `learning_rate` | 5e-6 | About 40x lower than SFT to avoid catastrophic forgetting |
| `per_device_batch_size` | 2 | DPO needs 4 forward passes per step (chosen+rejected × model+ref); about 10GB at this sequence length |
| `gradient_accumulation_steps` | 4 | Effective batch size = 8 |
| `max_seq_length` | 2048 | Compact observations (pruned AXTree, no HTML) plus the short DPO prompt fit here |
| `max_prompt_length` | `max_seq_length - 512` | Set in `dpo_train.py`; actions are short (about 50 tokens) |
| `num_epochs` | 2 | |
| `ref_model` | `null` | Adapter-as-reference: TRL's `DPOTrainer` with `ref_model=None` and a PEFT config, so no separate reference model is loaded |

### M-GRPO

| Parameter | Value | Notes |
|-----------|-------|-------|
| `kl_coefficient` | 0.001 | Low, so the policy can diverge from SFT |
| `clip_epsilon` | 0.2 | PPO-style clipping |
| `group_size` | 8 | Rollouts per challenge for advantage estimation (the `--group-size` CLI flag defaults to 4) |
...
3. **Multi-turn conversation**: Maintains conversation history within each step, matching the training format:
   ```
   system prompt
...
4. **Context management**: Monitors token count and trims the oldest conversation turns (keeping the first user message with the task instruction) to stay within `max_seq_length`. This avoids right-truncation, where the tokenizer would chop the most recent observation.

5. **Step transitions**: Detects step changes from the task prompt and resets conversation history.

6. **Generation**: `model.generate()` with `repetition_penalty=1.15` to prevent action loops, and two EOS tokens (`<|im_end|>` + `<|endoftext|>`) for stopping.
...
`data/`, `models/` and `results/` are gitignored, so the repository does not include trajectories, checkpoints or metrics. The figures below come from local runs.
...
| 26-30 | Not attempted | Not attempted | Need steps 13-25 first |

### Lessons Learned

- **SFT loss 0.85 → 0.05** over 5 epochs with 115 expert trajectories. The model generates valid BrowserGym actions but has JavaScript knowledge gaps (e.g., `NodeList.map()` instead of `Array.from(...).map()`).
...
- **DPO sequence length has to fit the observations**: An earlier run with full observations (about 2500 tokens plus a 1522-token system prompt) at `max_seq_length` 2048 truncated everything. The current config uses compact observations (pruned AXTree, no HTML) so that 2048 fits.
- **Context window management for local inference**: After about 5 multi-turn exchanges, the conversation exceeds `max_seq_length`. Without active trimming, right-truncation chops the latest observation and the model degenerates into outputting observation-like text.
